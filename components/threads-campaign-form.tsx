...
export function ThreadsCampaignForm({ campaignId }: { campaignId?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/threads/posts?threadsAccountId=${encodeURIComponent(accountId)}`)
      .then((response) => response.json())
      .then((payload) => setPosts(payload.success ? payload.data : []));
  }, [accountId]);
  ...
  return (
    <form onSubmit={submit} className="panel space-y-6 rounded p-5 sm:p-7">
      ...
      <label className="block text-sm font-semibold">Threads account
        <select className="mt-2 w-full rounded border border-border bg-surface px-4 py-3" value={accountId} onChange={(e) => { setAccountId(e.target.value); setPostId(""); }} required disabled={Boolean(campaignId)}>
      ...
      <button disabled={saving || accounts.length === 0} className="rounded bg-accent px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Saving…" : "Save Threads campaign"}</button>
    </form>
  );
}
