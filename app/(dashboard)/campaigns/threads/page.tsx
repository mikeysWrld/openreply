...
type Campaign = {
  id: string; name: string; postUrl: string; keywords: string[]; replyMessage: string;
...
export default function ThreadsCampaignsPage() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="panel h-40 rounded" /> : campaigns.length === 0 ? <div className="panel rounded p-10 text-center"><p className="font-semibold">No Threads campaigns yet</p><p className="mt-2 text-sm text-muted">Connect Threads in Settings, then choose a post to monitor.</p></div> : <div className="space-y-3">{campaigns.map((campaign) => <article key={campaign.id} className="panel rounded p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="font-bold">{campaign.name}</h2><span className="rounded-full border border-border px-2 py-0.5 text-xs text-muted">@{campaign.threadsAccount.username}</span><span className={campaign.isActive ? "text-xs text-success" : "text-xs text-muted"}>{campaign.isActive ? "Active" : "Paused"}</span></div><div className="mt-3 flex flex-wrap gap-1.5">{campaign.keywords.map((keyword) => <span key={keyword} className="rounded bg-accent/10 px-2 py-1 text-xs text-accent">{keyword}</span>)}</div><p className="mt-3 text-sm text-muted">“{campaign.replyMessage}”</p><p className="mt-3 text-xs text-muted">{campaign.analytics.sent} sent · {campaign.analytics.failed} failed · {campaign.analytics.pending} pending</p></div><div className="flex gap-2"><a href={campaign.postUrl} target="_blank" rel="noreferrer" className="rounded border border-border px-3 py-2 text-xs">View post</a><Link href={`/campaigns/threads/${campaign.id}/edit`} className="rounded border border-border px-3 py-2 text-xs">Edit</Link><button onClick={() => void toggle(campaign)} className="rounded border border-border px-3 py-2 text-xs">{campaign.isActive ? "Pause" : "Activate"}</button></div></div></article>)}</div>}
  </div>;
}
